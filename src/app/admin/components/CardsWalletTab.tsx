...
export default function CardsWalletTab({ adminKey }: { adminKey: string }) {
  ...
  const [section, setSection] = useState<"cards" | "wallet">("cards");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-dark-400 text-sm mt-1">View member cards, approve wallet top-ups, and credit wallets manually</p>
      </div>

      {/* Secondary nav */}
      <div className="flex items-center gap-2 p-1 bg-dark-900/80 border border-dark-800/50 rounded-xl w-fit">
        {(["cards", "wallet"] as const).map((s) => (
          <button
            key={s}
          ...
          >
            <span className="flex items-center gap-2">
              {s === "cards" ? <CreditCard size={14} /> : <Wallet size={14} />}
              {s === "cards" ? "User Cards" : "Wallet Top-Up"}
            </span>
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <div className="border-t border-dark-800/50 pt-6">
            ...
            {usersLoading ? (
              ...
              <div className="text-center py-16 bg-dark-900/60 border border-dark-800/50 rounded-2xl">
                <Users className="w-12 h-12 text-dark-600 mx-auto mb-3" />
                <p className="text-dark-400 text-sm">No matching users</p>
              </div>
            ) : (
              <div className="space-y-3">
                {walletFiltered.map((u) => {
                  ...
                  return (
                    ...
                    >
                      <button
                        onClick={() => setWalletExpanded(isExpanded ? null : u.id)}
                        className="w-full px-4 sm:px-6 py-4 flex items-center gap-4 hover:bg-white/[0.02] transition-colors text-left"
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-gradient-to-br ${meta.avatar} border border-dark-700/50`}>
                          <span className={`text-sm font-bold ${meta.initial}`}>
                        ...
                        <div className="px-4 sm:px-6 pb-6 space-y-5 border-t border-dark-800/30 pt-5">
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border border-emerald-500/20 rounded-xl p-4">
                              ...
                              <p className="text-xl font-bold text-emerald-400 mt-1">{formatPrice(u.walletBalance ?? 0)}</p>
                            </div>
                            <div className="bg-gradient-to-br from-sky-500/10 to-sky-500/5 border border-sky-500/20 rounded-xl p-4">
                              <p className="text-xs text-dark-400">Peak Balance</p>
                              <p className="text-xl font-bold text-sky-400 mt-1">{formatPrice(u.peakWalletBalance ?? 0)}</p>
                            </div>
                            <div className="bg-gradient-to-br from-gold-500/10 to-gold-500/5 border border-gold-500/20 rounded-xl p-4">
                              <p className="text-xs text-dark-400">Card Level</p>
                              <p className="text-base font-bold text-gold-300 mt-2">{meta.name || "None"}</p>
                            </div>
                          </div>
                          ...
                          <div className="bg-gradient-to-br from-blue-500/10 to-blue-500/5 border border-blue-500/20 rounded-xl p-4 space-y-3">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                              <input
                                ...
                                className="flex-1 px-4 py-2.5 bg-dark-800/60 border border-dark-700/50 rounded-xl text-white text-sm placeholder:text-dark-500 focus:outline-none focus:border-gold-500/50"
                              />
                              <button
                                onClick={() => handleCredit(u.id, amount)}
                                disabled={creditingId === u.id || !amount.trim()}
                                className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-white transition-all disabled:opacity-50"
                              >
                                {creditingId === u.id ? (
