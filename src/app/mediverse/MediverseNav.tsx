...
export default function MediverseNav({
  active,
  onCategoryChange,
  subActive,
  onSubChange,
  ...
  const [categories, setCategories] = useState(DEFAULT_MEDIVERSE_CATEGORIES);
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      if (data?.mediverse && Array.isArray(data.mediverse) && data.mediverse.length > 0) {
        const cats = [{ id: "all", label: "All" }, ...data.mediverse.map((c: { slug: string; name: string; subcategories: { slug: string; name: string }[] }) => ({
          id: c.slug,
          label: c.name,
        }))];
        setCategories(cats);
        const subs: Record<string, { id: string; label: string }[]> = {};
        for (const c of data.mediverse) {
